...
/**
 * Controller for day list panel with calendar event model.
 * Maintains a map of eventID to eventLabel for each all day event on a given 
 * day. When the calendar event model or date is changed, the map is updated and
 * the dayListPanel is notified.
 * 
 * @author Jeremy Moore
 */
public final class DayListCtlr {
    ...
    public DayListCtlr(CalModel calModel, DateCtlr dateCtlr) {
        ...
        this.eventMap = new HashMap<EventID, EventLabel>();        
        this.initEventMap();
    ...
    private void initEventMap() {
        eventMap.clear();
        for (CalEvent event : calModel.getCalEventsAllDay(dateCtlr.getDay())) {
